...
### 1.3 서버가 클라이언트보다 두꺼운 이유
교수님 지침대로, **백엔드(서버)가 훨씬 두껍고 클라이언트는 가볍게** 설계했습니다. 서버는 소켓 관리·메시지 파싱·검증·라우팅·파일 CRUD·동시성 제어를 모두 담당하고, 클라이언트는 "사용자 입력을 HTTP 요청 문자열로 바꿔 보내고 응답을 출력"하는 얇은 역할만 합니다.
...
> **동기**: 교수님 조언 — *"CREATE를 '기존 파일에 데이터를 생성하는 방법'과 '새 파일을 만들어서 데이터를 생성하는 방법', 둘 다 유연하게 할 수 있으면 좋다."* (상대평가에서 중간·기말 동점 시 **과제 성실도**로 순위를 가르기 때문에, 유연한 구현으로 성실도를 어필)
