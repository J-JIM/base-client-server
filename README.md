...
### 2.2 v1.1 (6/28) — 기능 추가: CRUD를 이용한 파일·데이터 생성/삭제 ⭐
...
실제 서버를 띄워 아래를 **자동 테스트**로 확인했습니다.

**(1) 동시 접속(concurrent) 확인**
- 클라이언트 A의 연결을 **열어 둔 상태**에서 클라이언트 B가 **즉시 응답**을 받음 → iterative였다면 B는 타임아웃. ✅

**(2) Lock 경쟁 조건 확인**
- **20개 스레드가 동시에** 같은 파일에 POST → **최종 20줄, id 전부 유니크**(유실·중복 0). ✅
...
### 발표 때 짚으면 좋은 것 (평가 포인트)
- 프레임워크 없이 **소켓 위 HTTP 직접 구현**
- **상태코드 10종 전부** (성공 + 오류 골고루)
- **파일 단위 + 레코드 단위 CRUD** (CREATE 두 갈래: 새 파일 / 기존 파일)
- **동시 클라이언트(스레드) + Lock(파일 안전)**
- Wireshark로 **HTTP가 TCP 위에 얹힌 계층 구조** 확인

### ⚠️ 주의사항
- ⭐ **명령어·띄어쓰기는 "영문(한/영) 입력 상태"에서 치기** — 한글 입력기로 치면 스페이스가 **전각 공백**으로 들어가 명령이 안 먹힐 수 있음(`404 빈 경로`). 이름(김주한)만 한글로. *(client가 자동 정규화하도록 보강돼 있지만, 영문 스페이스가 가장 확실)*
...
| 항목 | 링크 |
|------|------|
| 🎥 시연 영상 (YouTube) | https://youtu.be/여기에_영상_링크_붙여넣기 |
| 💻 GitHub 저장소 | https://github.com/J-JIM/base-client-server |

> ※ 유튜브 업로드 후 **이 표의 링크**와 **문서 맨 위 링크**를 실제 주소로 교체하세요.
